...
The `npm run:production` command will run the [build Gulp task][gulp-task-build-production] that deletes any existing production builds, and builds a new production-ready version of the website. The `npm run deploy` will run the [deploy build task][gulp-task-deploy] that uses [rsync][rsync] to upload the website to the specified remote server.
...
[gulp-task-build-production]: tools/gulp/tasks/production/build.js
